...
func (s *KnowledgeBaseService) UploadDocument(ctx context.Context, userID, kbID uuid.UUID, filename string, fileSize int64, reader interface{ Read([]byte) (int, error) }) (*domain.Document, error) {
	...
	fileURL, err := s.minio.Upload(ctx, "rag-documents", key, uploadReader, fileSize, "application/octet-stream")
	if err != nil {
	...
	// Process document asynchronously (in production, use a worker)
	go s.processDocument(context.Background(), kb, doc)

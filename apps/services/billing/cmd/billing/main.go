// Package main is the entry point for the Billing Service.
package main
...
import (
	...
	"github.com/omnidev/services/billing/internal/handler"
...
func main() {
	...
	logger.Log.Info("Starting Billing Service", zap.String("version", version), zap.String("env", cfg.App.Env))

	ctx := context.Background()

	// Telemetry
	shutdownTel, err := telemetry.Init(ctx, "billing-service", version, cfg.Telemetry.OTLPEndpoint)
	if err != nil {
	...
	wechatProvider := payment.NewWechatProvider()
	alipayProvider := payment.NewAlipayProvider()
	...
	// Services
	billingSvc := service.NewBillingService(accountRepo, usageRepo, invoiceRepo, stripeProvider, wechatProvider, alipayProvider)
	...
	v1.Use(middleware.JWTAuth(jwtManager))
	{
		...
		{
			billing.GET("/account", billingHandler.GetAccount)
			billing.GET("/usage", billingHandler.GetUsage)
			billing.POST("/usage/record", billingHandler.RecordUsage)
			...
			billing.POST("/payment-method", billingHandler.AddPaymentMethod)
		}
	}

	addr := fmt.Sprintf(":%d", cfg.App.Port+7) // Billing on 8088
	srv := &http.Server{Addr: addr, Handler: r, ReadTimeout: 15 * time.Second, WriteTimeout: 300 * time.Second}

	go func() {
		logger.Log.Info("Billing Service starting", zap.String("addr", addr))
	...
	logger.Log.Info("Shutting down Billing Service...")
	shutdownCtx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	_ = srv.Shutdown(shutdownCtx)
	logger.Log.Info("Billing Service exited")
}
